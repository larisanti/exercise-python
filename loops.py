...
Objective:
Develop functions for organizing and calculating student exam scores.

Notes:
- while loops -> indefinite = uncounted iteration
- for loops -> definite = counted iteration
- customize loop behavior: else, continue, break
- counting and indexing: range(), enumerate()
- range -> lazy objects = só consomem memória quando chamado (quase nada de espaço)
^ pois não cria todos os nros de uma vez na memória
- enumerate -> if both values and indexes are needed
^ can be used as lookup if lengths and indexes line up
- pop() method -> removes and returns the last item in a list
"""
